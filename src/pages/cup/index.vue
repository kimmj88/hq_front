  ...
  <v-container>
    ...
    >
      <template #item.name="{ item }">
        <router-link :to="CUP_PATH.VIEW(item.id)" class="account-link">
          {{ item.name }}
        </router-link>
      </template>
...
async function loadItems(options: FetchParams) {
  try {
    ...
    const response = await api.get(
      `${getBaseUrl('DATA')}/cup/search?keyword=${search.value}&page=${options.page}&itemsPerPage=${
        options.itemsPerPage
      }&sortBy=${sortKey}&orderBy=${sortOrder}`
    );
