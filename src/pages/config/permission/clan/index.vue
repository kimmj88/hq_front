                  ...
                  <v-col v-for="(perm, pIdx) in group.children" :key="perm.code" cols="12" sm="6">
                    <v-checkbox
                      ...
                      :disabled="
                            ...
                            "
                      :label="$t('clan_permission.' + perm.code)"
                      hide-details
                      density="compact"
                    />
                  </v-col>
...
const search = ref('');
const serverItems = ref<ClanRole[]>([]);
...
const selectedRole = ref<ClanRole | null>(null);
...
// edit 클릭 → 다이얼로그 열기
async function editItem(item: ClanRole) {
  isCreating.value = false;
  selectedRole.value = item;
  try {
    const response = await api.get(`${getBaseUrl('DATA')}/clanrole/find?id=${item.id}`);
...
async function openCreateDialog() {
  ...
  try {
    const response = await api.get(`${getBaseUrl('DATA')}/clanpermission/all`);
...
async function savePermissions() {
  try {
    if (isCreating.value) {
      ...
      await api.post(`${getBaseUrl('DATA')}/clanrole/create`, {
        name: inputRoleName.value,
        permissionGroups: editedPermissions.value,
      });
      inputRoleName.value = '';
    } else {
      await api.post(`${getBaseUrl('DATA')}/clanrole/sync`, {
...
async function deleteItem(item: ClanRole) {
  const confirmed = confirm(`정말로 "${item.name}" 역할을 삭제하시겠습니까?`);
  if (!confirmed) return;

  try {
    await api.post(`${getBaseUrl('DATA')}/clanrole/delete`, {
      id: item.id,
    });
...
async function loadItems(options: any) {
  try {
    ...
    const response = await api.get(
      `${getBaseUrl('DATA')}/clanrole/search?keyword=${search.value}&page=${
        options.page
